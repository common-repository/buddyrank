...
class BP_Loop_Filters {
	...
	function bp_activity_paged_activities_sql($sql)
	{
		...
		if(!$diable_rank && class_exists('VoterPluginClass')){
			...
			$lasttime_hrs = get_option('azbr_lasttime_hrs');
			$lasttime_days = get_option('azbr_lasttime_days');
			if($lasttime_days>0){
				$mysqlTimeDiffVar = 'DAY';
				$mysqlTimeDiffVal = $lasttime_days;
			}else{
				if(!$lasttime_hrs){$lasttime_hrs=24;}
				$mysqlTimeDiffVar = 'HOUR';
				$mysqlTimeDiffVal = $lasttime_hrs;
			}
			...
			$select_subsql = ",( $photo_sql  $follow_sql ((select count(v.id) from ".$table_prefix."ask_votes v where v.secondary_item_id=a.id and v.action='up' and v.type='activity' and v.component='buddypress')*".$votedcount_weightage.")+IF(TIMESTAMPDIFF(".$mysqlTimeDiffVar.", '".$now_date."', a.date_recorded) >".$mysqlTimeDiffVal.", 0, (".$last24hr_weightage."/(0.01+TIMESTAMPDIFF(".$mysqlTimeDiffVar.", a.date_recorded, '".$now_date."'))))+(length(a.content)-length(replace(a.content,' ',''))+".$contentlength_weightage.")) as score";
